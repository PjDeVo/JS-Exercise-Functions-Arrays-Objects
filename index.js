...
const pat = makePersonObject(1, "Pat", "Something");
console.log(pat);
...
nick = makeSmartPerson("Nick");
console.log(nick);
console.log(nick.speak());
console.log(nick.sum(3, 4));
...
const honda = getCarInfoByIndex(inventory, 5);
console.log(honda);
 ...
 */
function getLastCarInfo(listOfCars) {
  return `This is a ${listOfCars[listOfCars.length - 1].car_make} ${
    listOfCars[listOfCars.length - 1].car_model
  }`;
}
const dodge = getLastCarInfo(inventory);
console.log(dodge);
 ...
 * it will return `This is a Lincoln Navigator`.
 */
function getCarInfoById(list, index) {
...
const idFunc = getCarInfoById(inventory, 5);
console.log(idFunc);
 ...
 */
function sortCarInventory(list) {
...
// const alphabeticalOrder = sortCarInventory(inventory);
 ...
 */
function getModelYears(list) {
  let carYears = [];
  for (i = 0; i < list.length; i++) {
    carYears.push(list[i].car_year);
  }
  return carYears;
}
 ...
 */
function getOlderCars(list, maxYear) {
  let carYears = [];
  for (i = 0; i < list.length; i++) {
    if (list[i].car_year <= maxYear) {
      carYears.push(list[i]);
    }
  }
  return carYears;
}

let olderCars = getOlderCars(inventory, 1999);
...
function getGermanCars(list) {
  ...
  for (i = 0; i < list.length; i++) {
    if (
      list[i].car_make === "Audi" ||
      list[i].car_make === "Mercedes-Benz" ||
      list[i].car_make === "Volkswagen" ||
      list[i].car_make === "BMW"
    ) {
...
let germanCars = getGermanCars(inventory);
...
const jeep = carMaker(200);
console.log(jeep);
console.log(jeep.drive(100));
